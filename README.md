...
``` 
## Pile-up plots
Aggregate plots of loops in DMSO or dTAG-7 treated HEK293T was plotted by coolpup.py. The loops were from Footprint-C in WT HEK293T, and were aggregated at the center of a ±200 kb window at 5-kb resolution. The detailed implementation process is in Jypyter notebook.
```
analysis/pileup_loop.ipynb
```

Aggregation plot centered at DNase-seq specific, common, or Footprint-C specific peak summits was also plotted by coolpup.py.. The contact matrices were aggregated around summits ±1000 kb at 10-kb resolution. The detailed implementation process is in Jypyter notebook. 
```
analysis/pileup_local.ipynb
```
...
The script is
```
analysis/motif_uniqueness_analysis.sh
``` 
...
The script is
```
analysis/motif_orientation_of_CTCF_CTCF_contact_in_pair.sh
``` 
...
The script is
```
analysis/heatmap_based_motif.sh
``` 
...
The script is
```
analysis/stratum_adjusted_correlation_coefficient.sh
``` 

## Reference
Liu, X.#, Wei, H.#, Zhang, Q., Zhang, N., Wu, Q., Xu, C. Footprint-C reveals transcription factor modes in local clusters and long-range chromatin interactions. _Nat Commun_ 15, 10922 (2024). https://www.nature.com/articles/s41467-024-55403-7
